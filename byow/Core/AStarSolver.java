...
public class AStarSolver<Vertex> implements ShortestPathsSolver<Vertex> {
    ...
    private final ArrayHeapMinPQ<Vertex> PQ;
    ...
    public AStarSolver(AStarGraph<Vertex> graphGiven, Vertex start, Vertex goal, double timeOut) {
        ...
        //add the source
        PQ.add(start, 0);
        //distTo is the best know distance to a node
        distTo = new HashMap<>();
        //edgeTo is the best predecessor
        edgeTo = new HashMap<>();
        ...
        while (PQ.size() > 0 && timeSpent < timeOut && !PQ.getSmallest().equals(goal)) {
            //Gets the vertex from the pq
            Vertex p = PQ.removeSmallest();
            //get all its neightbors
    ...
    public void relaxEdge(ArrayHeapMinPQ<Vertex> PQ, WeightedEdge<Vertex> neighbor, Vertex goal) {
        ...
        //the other vertex connected to us
        Vertex qTo = neighbor.to();
        //get the weight between them
        double weight = neighbor.weight();


        //This has to exist right?
        //get the distance to our current node
        double distanceToP = distTo.get(pFrom);
        //if distTo does not contain it
        if (!distTo.containsKey(qTo)) {
            //relax and update PQ
            //update distTo and Edge to and  PQ, so we have new best know distance and predecessor
